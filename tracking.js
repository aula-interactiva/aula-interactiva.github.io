(() => {
  ...
  function jsonp(params = {}) {
    return new Promise((resolve, reject) => {
      const callback = '__aulaJsonp_' + Date.now().toString(36) + Math.random().toString(36).slice(2);
  ...
  function saveSession(id, role, token) {
    const session = {id: normalizeId(id), role, token: String(token || ''), loggedAt: Date.now()};
  ...
  async function validateId(value) {
    ...
    try {
      ...
      if (!result?.ok) return {ok: false, id, reason: result?.error === 'rate-limited' ? 'rate-limited' : 'not-found'};
      return {
        ok: true,
        id: normalizeId(result.id),
        role: result.role === 'teacher' ? 'teacher' : 'student',
  ...
  window.PracticeTracker = Object.freeze({
    ...
    apiGet(action, params = {}) {
      const session = getSession();
      if (!session?.token) return Promise.resolve({ok: false, error: 'no-session'});
      return jsonp({action, token: session.token, ...params});
    }
  });
})();
